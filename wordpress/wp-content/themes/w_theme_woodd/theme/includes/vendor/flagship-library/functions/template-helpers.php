...
function flagship_get_entry_comments_link( $args = array() ) {
	...
	$defaults = apply_filters( 'flagship_entry_comments_link_defaults',
		array(
			...
			'before'      => '',
			'hide_if_off' => 'enabled',
			'more'        => __( '% Comments', 'w_theme_woodd' ),
			'one'         => __( '1 Comment', 'w_theme_woodd' ),
			'zero'        => __( 'Leave a Comment', 'w_theme_woodd' ),
		)
	);
	$args = wp_parse_args( $args, $defaults );
	...
	while ( $counter <= $footer_widgets ) {
		hybrid_register_sidebar(
			array(
				...
				'name'        => sprintf( __( 'Footer %d', 'w_theme_woodd' ), $counter ),
				'description' => sprintf( __( 'Footer %d widget area.', 'w_theme_woodd' ), $counter ),
			)
		);
